...
set "SONIC_LAUNCH_LOG=%SONIC_LOG_DIR%\launcher.log"
echo [%date% %time%] SonicScout launcher started>>"%SONIC_LAUNCH_LOG%"
...
goto sync
:build
dotnet restore "%PROJECT%" --verbosity quiet 2>>"%SONIC_LAUNCH_LOG%"
if errorlevel 1 goto fail
dotnet build "%PROJECT%" --configuration Release --no-restore --verbosity minimal 2>>"%SONIC_LAUNCH_LOG%"
if errorlevel 1 goto fail
:sync
...
goto finish_sync
:build_scoutpass
dotnet restore "%SCOUTPASS_PROJECT%" --verbosity quiet 2>>"%SONIC_LAUNCH_LOG%"
if errorlevel 1 goto fail
dotnet build "%SCOUTPASS_PROJECT%" --configuration Release --no-restore --verbosity minimal 2>>"%SONIC_LAUNCH_LOG%"
if errorlevel 1 goto fail
:finish_sync
