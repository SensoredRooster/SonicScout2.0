...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/health") {
      return json({ ok: true, service: "sonicscout2-support-collector" });
    }
    if (request.method === "POST" && url.pathname === "/upload") {
      ...
      if (bytes.length < 2 || bytes[0] !== 0x50 || bytes[1] !== 0x4b) return json({ error: "Expected a ZIP support bundle" }, 415);
      const session = safeName(request.headers.get("x-sonicscout-session"), "unknown-session");
      const original = safeName(request.headers.get("x-sonicscout-filename"), "SonicScout2.0-Support.zip");
    ...
    if (request.method === "GET" && url.pathname === "/bundles") {
      ...
      return json({ bundles: listed.objects.map(obj => ({ name: obj.key.split("/").pop(), size_bytes: obj.size, uploaded: obj.uploaded })) });
    }
