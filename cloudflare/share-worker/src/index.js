...
async function signSession(env, role) {
  const payload = b64url(encoder.encode(JSON.stringify({ role, exp: Math.floor(Date.now()/1000)+SESSION_SECONDS, project: "sonicscout2" })));
...
async function verifySession(request, env) {
  ...
  try {
    ...
    if (payload.project !== "sonicscout2" || payload.exp < Math.floor(Date.now()/1000)) return null;
...
const LOGIN = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SonicScout2.0 Tester Share</title>
...
const PORTAL = `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SonicScout2.0 Tester Share</title>
...
<body><header><div class="brand"><strong>SonicScout2.0 Tester Share</strong><span id="role"></span></div><button id="logout">Sign out</button></header><div class="wrap"><div id="latest" class="latest hidden"></div><div class="grid"><aside class="panel folders" id="folders"></aside><section class="panel"><div class="toolbar"><input id="file" type="file"><input id="desc" placeholder="Optional description"><button class="primary" id="upload">Upload</button><span class="muted" id="status"></span></div><div class="progress hidden" id="progress"><i></i></div><table class="files"><thead><tr><th>Name</th><th>Size</th><th>Uploaded</th><th>Description</th><th>Actions</th></tr></thead><tbody id="rows"></tbody></table></section></div></div>
...
export default { async fetch(request, env) {
  ...
  if (request.method === "GET" && url.pathname === "/health") return json({ ok:true, service:"sonicscout2-tester-share", storage:"r2" });
